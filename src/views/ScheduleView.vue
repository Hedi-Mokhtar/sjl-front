<template>
  <div class="min-h-screen bg-gradient-to-br from-gray-50 to-red-50 dark:from-gray-900 dark:to-gray-800">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">

      <!-- Header -->
      <section class="text-center mb-8 sm:mb-12">
        <h1 class="text-3xl sm:text-4xl md:text-5xl font-bold text-gray-800 dark:text-gray-200 mb-4">
      ...
      <div class="hidden lg:block mb-12">
        <div class="bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-200 dark:bg-gray-800 dark:border-gray-700">
          <div class="overflow-x-auto">
            <table class="min-w-full">
              ...
              <tbody>
                <tr v-for="(team, index) in teams" :key="index" :class="index % 2 === 0 ? 'bg-gray-50 dark:bg-gray-700' : 'bg-white dark:bg-gray-800'"
                  class="border-b border-gray-200 dark:border-gray-600 hover:bg-red-50 dark:hover:bg-gray-600 transition-colors">
                  <td class="py-4 px-6">
                    <div class="font-semibold text-gray-800 dark:text-gray-200">{{ team.name }}</div>
                    <div v-if="team.details" class="text-sm text-gray-500 dark:text-gray-400 mt-1">{{ team.details }}</div>
                  </td>
                  <td class="py-4 px-6">
                    <div v-for="(schedule, idx) in team.schedules" :key="idx" class="mb-1">
                      <span class="text-gray-700 dark:text-gray-300">{{ schedule }}</span>
                    </div>
                  </td>
                  <td class="py-4 px-6">
      ...
      <div class="block lg:hidden space-y-4 mb-12">
        <div v-for="(team, index) in teams" :key="index"
          class="bg-white rounded-xl shadow-lg overflow-hidden border border-gray-200 dark:bg-gray-800 dark:border-gray-700 transition-all hover:shadow-xl">
          ...
          <div class="p-4 space-y-4">
            ...
            <div>
              <div class="flex items-center gap-2 mb-2">
                ...
                <span class="font-semibold text-gray-800 dark:text-gray-200">Horaires</span>
              </div>
              <div class="space-y-1 ml-0">
                <div v-for="(schedule, idx) in team.schedules" :key="idx"
                  class="text-sm text-gray-700 dark:text-gray-300 bg-gray-50 dark:bg-gray-700 px-3 py-2 rounded-lg">
                  {{ schedule }}
                </div>
              </div>
            </div>
            ...
            <div class="flex items-center justify-between pt-3 border-t border-gray-200 dark:border-gray-600">
              <div class="flex items-center gap-2">
                ...
                <span class="font-semibold text-gray-800 dark:text-gray-200">Tarif</span>
              </div>
      ...
      <!-- Image des horaires -->
      <section class="mb-12">
        <div class="bg-white rounded-2xl shadow-xl p-6 sm:p-8 dark:bg-gray-800">
      ...
      <!-- Section Contact -->
      <section class="bg-white rounded-2xl shadow-xl p-6 sm:p-8 text-center dark:bg-gray-800">
